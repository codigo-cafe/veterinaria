	...
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				...
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Atenciones</h6>
					<div class="d-flex justify-content-end">
						<Link :href="route('atenciones.create')" class="btn btn-blue">
							<i class="fas fa-plus"></i> Registrar Atención
						</Link>
					</div>
					<div class="row">
					...
					<div class="table-responsive">
						<table class="table align-items-center mb-4" id="datatable">
							<thead class="thead-light">
								<tr>
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
										<a href="#" @click="sort('id_aten')" class="w-100 d-inline-flex justify-content-between text-secondary">#</a>
										<i v-if="params.field ==='id_aten' && params.direction === 'asc'" class="fas fa-sort-down fa-lg"></i>
										<i v-if="params.field ==='id_aten' && params.direction === 'desc'" class="fas fa-sort-up fa-lg"></i>
									</th>
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
							...
							<tbody>
								<tr v-for="atencion in atenciones.data" :key="atencion.id_aten">
									<td class="text-sm font-weight-normal">{{ atencion.id_aten }}</td>
									<td class="text-sm font-weight-normal">{{ atencion.mascota.nom_masc }}</td>
									<td class="text-sm font-weight-normal">{{ atencion.mascota.raza_masc }}</td>
									<td class="text-sm font-weight-normal">{{ atencion.mascota.sexo_masc }}</td>
									<td class="text-sm font-weight-normal">{{ atencion.mascota.color_masc }}</td>
									<td class="text-sm font-weight-normal">{{ atencion.mascota.espe_masc }}</td>
									<td class="text-sm font-weight-normal">{{ atencion.mascota.edad_masc }}</td>
									<td class="text-sm font-weight-normal">{{ fecha(atencion.fech_aten) }}</td>
									<td class="text-sm">
										<div class="d-flex justify-content-center align-items-center">
											<Link :href="route('atenciones.show', atencion.id_aten)" class="btn btn-icon-only btn-rounded btn-outline-info mb-0 me-2 btn-md d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" data-bs-original-title="Mostrar">
												<i class="material-icons text-lg">visibility</i>
											</Link>
											<Link :href="route('atenciones.edit', atencion.id_aten)" class="btn btn-icon-only btn-rounded btn-outline-warning mb-0 me-2 btn-md d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" data-bs-original-title="Editar">
												<i class="material-icons text-lg">edit</i>
											</Link>
											<button type="button" @click="destroy(atencion)" class="btn btn-icon-only btn-rounded btn-outline-danger mb-0 me-2 btn-md d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" data-bs-original-title="Eliminar">
												<i class="material-icons text-lg">clear</i>
											</button>
										</div>
									</td>
								</tr>
								<tr v-if="atenciones.data.length == 0">
									<td class="text-center" colspan="100%">No se encontraron resultados</td>
								</tr>
							</tbody>
						</table>
						<pagination class="mt-4 mb-2" :links="atenciones.links" :results="atenciones" />
					</div>
					<!--  end card  -->
				</div>
				<!-- end col-md-12 -->
...
export default {
	...
	data() {
		return {
			params: {
				...
				field: this.filters.field != null ? this.filters.field : 'id_aten',
				direction: this.filters.direction != null ? this.filters.direction : 'asc',
			},
			infomodal: {
				id: null,
				nombre: null,
				mensaje: "Seguro de eliminar la atención de la mascota: ",
				url: "atenciones.destroy",
			},
		}
	},
	...
	watch: {
		params: {
			handler() {
				this.$inertia.get(this.route('atenciones.index'), this.params, { replace: true, preserveState: true });
			},
			deep: true,
		},
	},
	...
	methods: {
		...
		destroy(atencion){
			this.infomodal.id = atencion.id_aten;
			this.infomodal.nombre = atencion.mascota.nom_masc;
