	...
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				...
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Citas</h6>
					<div class="d-flex justify-content-end">
						<Link :href="route('citas.create')" class="btn btn-blue">
					...
					<div class="table-responsive">
						<table class="table align-items-center mb-4" id="datatable">
							<thead class="thead-light">
								<tr>
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
										<a href="#" @click="sort('id_cita')" class="w-100 d-inline-flex justify-content-between text-secondary">#</a>
										<i v-if="params.field === 'id_cita' && params.direction === 'asc'" class="fas fa-sort-down fa-lg"></i>
										<i v-if="params.field === 'id_cita' && params.direction === 'desc'" class="fas fa-sort-up fa-lg"></i>
									</th>
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
										<a href="#" @click="sort('tipo_cita')" class="w-100 d-inline-flex justify-content-between text-secondary">Tipo de Cita</a>
										<i v-if="params.field === 'tipo_cita' && params.direction === 'asc'" class="fas fa-sort-down fa-lg"></i>
										<i v-if="params.field === 'tipo_cita' && params.direction === 'desc'" class="fas fa-sort-up fa-lg"></i>
									</th>
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
										Celular
									</th>
							...
							<tbody>
								<tr v-for="cita in citas.data" :key="cita.id_cita">
									<td class="text-sm font-weight-normal">{{ cita.id_cita }}</td>
									<td class="text-sm font-weight-normal">{{ cita.tipo_cita }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.cliente.celular_clie }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.nom_masc }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.raza_masc }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.sexo_masc }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.color_masc }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.espe_masc }}</td>
									<td class="text-sm font-weight-normal">{{ cita.mascota.edad_masc }}</td>
									<td class="text-sm font-weight-normal">{{ fecha(cita.fec_cita) }}</td>
									<td class="text-sm">
										<div class="d-flex justify-content-center align-items-center">
											<Link :href="route('citas.show', cita.id_cita)" class="btn btn-icon-only btn-rounded btn-outline-info mb-0 me-2 btn-md d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" data-bs-original-title="Mostrar">
												<i class="material-icons text-lg">visibility</i>
											</Link>
											<Link :href="route('citas.edit', cita.id_cita)" class="btn btn-icon-only btn-rounded btn-outline-warning mb-0 me-2 btn-md d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" data-bs-original-title="Editar">
												<i class="material-icons text-lg">edit</i>
											</Link>
											<button type="button" @click="destroy(cita)" class="btn btn-icon-only btn-rounded btn-outline-danger mb-0 me-2 btn-md d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" data-bs-original-title="Eliminar">
						...
						<pagination class="mt-4 mb-2" :links="citas.links" :results="citas" />
					</div>
					<!--  end card  -->
				</div>
				<!-- end col-md-12 -->
...
export default {
	...
	data() {
		return {
			params: {
				...
				field: this.filters.field != null ? this.filters.field : 'id_cita',
				direction: this.filters.direction != null ? this.filters.direction : 'asc',
			},
			infomodal: {
				id: null,
				nombre: null,
				mensaje: "Seguro de eliminar la cita de la mascota: ",
				url: "citas.destroy",
			},
		}
	},
	...
	watch: {
		params: {
			handler() {
				this.$inertia.get(this.route('citas.index'), this.params, { replace: true, preserveState: true });
			},
			deep: true,
		},
	},
	...
	methods: {
		...
		destroy(cita){
			this.infomodal.id = cita.id_cita;
			this.infomodal.nombre = cita.mascota.nom_masc;
